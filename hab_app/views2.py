...
def home(request):
    ...
    if request.session['username'] == "chr_hab":
        ...
        return render(request,'hab_app/caretakerView.html',{'ROtable':ROtable,'permission':1})
        ...
        return render(request,'hab_app/caretakerView.html',{'ROtable':ROtable,'permission':0})
...
def ct_add_occupant(request):
    ...
    if request.method == 'GET':
        ...
        return render(request,'hab_app/ct_add_occupant.html',{'form1':form1,'form2':form2,'permission':'permission'})
            ...
            for i in all_rooms:
                ...
                if flag == 0:
                    if i.special_category != 0:
                        empty_rooms.append(i)
            hostels = AllHostelMetaData.objects.all()
            return render(request,'hab_app/chrViewSpecialRooms.html',{'ROtable':ROtable,'zipped':zipped ,'empty_rooms':empty_rooms,'permission2':permission2})
...
def chrFreshersBulkAllot(request):
    if request.method == 'GET':
        return render(request,'hab_app/FreshersBulkAllotUpload.html',{'permission2':1})
    ...
    if request.method=="POST":
        ...
        return render(request,'hab_app/chrView.html',{'permission2':1})
...
def caretakerapproveinfo(request):
    ...
    if request.method == 'GET':
        if request.GET.get('param') :
            ...
            return render(request,"hab_app/CTapproveEdit.html",{'form':form,'permission':permission,'ROtable':temp})
            ...
            #vgv
                ...
                for student in toapprove_info_list_total:
                    if hostelROModel.objects.filter(occupantId = student.idNo  ).count() == 1:
                        students_info_update.append(student)
                return render(request,"hab_app/caretaker_student_info_tobeupdated.html",{'students_info_update':students_info_update,'permission':1,'ROtable':temp})
        else:
            print(form.errors)

    return render(request,"hab_app/caretaker_student_info_tobeupdated.html",{'students_info_update':students_info_update,'permission':permission,'ROtable':temp})
...
def mess_automation(request):
    ...
    if request.method == 'GET':
        ...
        if request.GET.get('param2'):
                ...
                month = int(mth[0])
                year = int(mth[1])
                log = Automation.objects.get(month__iexact=month,year=year)
